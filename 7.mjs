import { run, parse, int, reduce } from './_utils'

const parser = (line) => {
	const [_, id, value, children] = line.match(/^(\w+) \((\d+)\)(?: -> ([\w ,]+))?$/)
	return { id, value: int(value), children: children ? children.split(', ') : [] }
}

const build = (nodes, { id, value, children }) => Object.assign(nodes, {
	[id]: Object.assign(
		nodes[id] || {},
		{ id, value, children: children.map((cid) => nodes[cid] ?
			Object.assign(nodes[cid], { parent: id }) :
			(nodes[cid] = { parent: id })
		) }
	)
})

const root = (tree) => Object.values(tree).find((node) => !node.parent)

run([{
	fn: (string) => root(reduce(parse(string, parser), build, {})).id,
